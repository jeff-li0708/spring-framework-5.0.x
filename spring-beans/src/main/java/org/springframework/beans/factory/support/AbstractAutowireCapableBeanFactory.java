...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		if (resolvedClass != null && !mbd.hasBeanClass() && mbd.getBeanClassName() != null) { //bd没设置beanClass,创建一个新的bd并设置beanClass
		...
		try {
			...
			// 给后置处理器一个返回代理对象的机会,会调用BeanPostProcessor的postProcessBeforeInitialization方法和postProcessAfterInitialization方法
	...
	protected Object resolveBeforeInstantiation(String beanName, RootBeanDefinition mbd) {
		Object bean = null;
		if (!Boolean.FALSE.equals(mbd.beforeInstantiationResolved)) { //后置处理器已启用
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
		// Make sure bean class is actually resolved at this point.
		//1.获取beanClass,确保bean class被解析
		Class<?> beanClass = resolveBeanClass(mbd, beanName);

		//2.验证beanClass不为空，且beanClass不是public,且没有权限访问构造函数和方法，抛出异常
		...
		if (instanceSupplier != null) {
			return obtainFromSupplier(instanceSupplier, beanName); //通过instanceSupplier获取一个bean实例
		}
		...
		if (args == null) {
			synchronized (mbd.constructorArgumentLock) {
				if (mbd.resolvedConstructorOrFactoryMethod != null) { //已解析的构造函数或工厂方法不为空
					resolved = true;
					autowireNecessary = mbd.constructorArgumentsResolved;//构造函数的参数是否已解析,默认是false,
				}
			}
		}
		if (resolved) {
			if (autowireNecessary) {//如果autowireNecessary为true说明是采用构造函数注入
		...
		//构造函数的参数args不为空或者第一次创建该bean
		Constructor<?>[] ctors = determineConstructorsFromBeanPostProcessors(beanClass, beanName);
		//构造函数列表不为空，或者设置了通过构造函数注入，或者构造函数参数列表不为空，或者传入的构造参数不为空
		if (ctors != null || mbd.getResolvedAutowireMode() == AUTOWIRE_CONSTRUCTOR ||
				mbd.hasConstructorArgumentValues() || !ObjectUtils.isEmpty(args)) {
			return autowireConstructor(beanName, mbd, ctors, args); //自动装配构造函数
		}
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		if (hasInstAwareBpps || needsDepCheck) {
			...
			if (hasInstAwareBpps) {
				for (BeanPostProcessor bp : getBeanPostProcessors()) {
					//如果有相关的后置处理器,进行相应的后置处理
					if (bp instanceof InstantiationAwareBeanPostProcessor) {
						InstantiationAwareBeanPostProcessor ibp = (InstantiationAwareBeanPostProcessor) bp;
						//对于标注了@Autowired的属性，将使用AutowiredAnnotationBeanPostProcessor后置处理器处理
						//对于标注了@@Resource的属性，将使用CommonAnnotationBeanPostProcessor后置处理器处理
	...
	protected void autowireByName(
		...
		String[] propertyNames = unsatisfiedNonSimpleProperties(mbd, bw); //获取需要填充的属性名
		for (String propertyName : propertyNames) {
			if (containsBean(propertyName)) {//propertyName对应的bean已存在单例缓冲池中或者beanDefinition
				Object bean = getBean(propertyName);//获取实例对象,可能是早期引用对象,存在循环依赖时，通过获取早期引用解决循环依赖问题
				pvs.add(propertyName, bean);
				registerDependentBean(propertyName, beanName);//记录依赖关系,dependentBeanMap和dependenciesForBeanMap集合的写入
				if (logger.isDebugEnabled()) {
	...
	protected void autowireByType(
		...
		for (String propertyName : propertyNames) {
			try {
				PropertyDescriptor pd = bw.getPropertyDescriptor(propertyName);//获取属性描述（包括属性名称、类型、writeMethodName、readMethodName等）
				// Don't try autowiring by type for type Object: never makes sense,
				// even if it technically is a unsatisfied, non-simple property.
				if (Object.class != pd.getPropertyType()) {
					// 获取相关的写方法参数(参数名、类型、写入方法信息)
	...
	protected String[] unsatisfiedNonSimpleProperties(AbstractBeanDefinition mbd, BeanWrapper bw) {
		...
		PropertyDescriptor[] pds = bw.getPropertyDescriptors(); //获取所有的属性描述（包括属性名称、类型、writeMethodName、readMethodName等）
		for (PropertyDescriptor pd : pds) {
			//属性的setter方法不为空，不在依赖检测排除名单中，没有属性值，且不是简单类型属性（如Number、Date等），
	...
	protected void applyPropertyValues(String beanName, BeanDefinition mbd, BeanWrapper bw, PropertyValues pvs) {
		...
		if (pvs instanceof MutablePropertyValues) {
			...
			if (mpvs.isConverted()) { // 如果已被设置转换完成，直接完成配置
		...
		for (PropertyValue pv : original) {
			if (pv.isConverted()) { //属性已转化，直接添加
				deepCopy.add(pv);
			}
			else {
				String propertyName = pv.getName();
				Object originalValue = pv.getValue();
				//获得转换后的属性值, 对于RuntimeReference，会解析拿到具体的beanName,最终通过getBean(beanName)拿到具体的对象
	...
	protected Object initializeBean(final String beanName, final Object bean, @Nullable RootBeanDefinition mbd) {
		if (System.getSecurityManager() != null) {
			...
			invokeAwareMethods(beanName, bean); //对实现了Aware接口的类初始一些属性
		}

		Object wrappedBean = bean;
		if (mbd == null || !mbd.isSynthetic()) {
			wrappedBean = applyBeanPostProcessorsBeforeInitialization(wrappedBean, beanName); //调用BeanPostProcessor.postProcessBeforeInitializationd对bean做一些后置处理
		}
